...
function buildFit(){
  ...
  if(missing.length){out.style.display='block';out.innerHTML='<b>Complete the operating questions to calculate your fit.</b><p>Company name and email are optional for the calculation and are only needed if you want the free reviewed shortlist.</p>';return;}
  ...
  const contactAction=(company&&email)
    ? `<a class="btn btn-primary" href="${mail}">Send for my free reviewed shortlist</a>`
    : `<a class="btn btn-primary" href="#fit-form" onclick="document.querySelector('[name=company]').focus();return true;">Add company + work email for the free shortlist</a>`;
  const contactNote=(company&&email)
    ? 'Nothing was transmitted by this calculation. Data is sent only if you choose to send the generated email. Buyer recommendations are free.'
    : 'Your result was calculated anonymously. Add company + work email above and calculate again only if you want the free reviewed shortlist or a vendor introduction.';
  out.style.display='block';out.innerHTML=`<span class="tag">${esc(stage)} · complexity ${complexity}</span><h2>Your preliminary software paths</h2><table><tr><th>Path</th><th>Fit</th></tr>${ranking.map(([k,v])=>`<tr><td>${esc(k==='Intuit'?'Intuit path (QBO / Enterprise Suite)':k)}</td><td><b>${v}/100</b></td></tr>`).join('')}</table><h3>Free 3-point fit note</h3><ol><li>${esc(note1)}</li><li>${esc(note2)}</li><li>${esc(note3)}</li></ol><div class="evidence"><b>Independence control:</b> commercial relationships are checked only after these scores are calculated. Money cannot change fit score or recommendation order.</div><div class="actions">${contactAction}</div><p class="fine">${esc(contactNote)}</p>`;out.scrollIntoView({behavior:'smooth',block:'start'});
}
